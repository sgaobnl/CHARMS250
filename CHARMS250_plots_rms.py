...
for tp in tps:
    for g in gains:
        ...
        #fn = f'D:/CHARMS250/data/BoardA11_ChipSN01_P2_RT/BGR_BL200_Noise_tp{tp}_gain{g}.bin'
        fn = f'D:/CHARMS250/data/RMS_0pF/BGR_BL200mV_Noise_tp{tp}_gain{g}.bin'
        with open (fn, 'rb') as fp:
            print (fn)
            rawdata = pickle.load(fp)
            chns = raw_conv(rawdata,  pktnum = 500000)[0]
            ...
            f, p = chn_fft(chns[9], fs = 2000000.0, fft_s = 5000, avg_cycle = 50)
            #plt.plot(f,p)
            #plt.plot(chns[9][0:1000])
            plt.grid()
            plt.show()
            plt.close()
